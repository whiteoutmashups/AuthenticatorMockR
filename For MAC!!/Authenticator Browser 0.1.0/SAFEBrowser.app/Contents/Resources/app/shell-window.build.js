...
// safe app plugin
electron.ipcRenderer.send('registerSafeApp');

electron.ipcRenderer.on('webClientAuthReq', function (event, uri) {
  handleSafeAuthAuthentication(uri, CLIENT_TYPES.WEB);
});

// safe authenticator plugin
electron.ipcRenderer.send('registerSafeNetworkListener');
electron.ipcRenderer.send('registerOnAuthReq');
electron.ipcRenderer.send('registerOnContainerReq');
electron.ipcRenderer.send('registerOnReqError');

electron.ipcRenderer.on('onNetworkStatus', function (event, status) {
...
electron.ipcRenderer.on('onAuthReq', function (event, data) {
  if (data) {
    safeAuthData = data;
    showSafeAuthPopup();
  }
...
electron.ipcRenderer.on('onContainerReq', function (event, data) {
  if (data) {
    safeAuthData = data;
...
electron.ipcRenderer.on('onAuthDecisionRes', function (event, data) {
...
electron.ipcRenderer.on('onContDecisionRes', function (event, data) {
  isSafeAppAuthenticating = false;
  if (data.type === CLIENT_TYPES.WEB) {
    electron.ipcRenderer.send('webClientContainerRes', data.res);
  }
  update();
});

electron.ipcRenderer.on('onAuthResError', function (event, data) {
  isSafeAppAuthenticating = false;
  if (data.type === CLIENT_TYPES.WEB) {
...
function handleSafeAuthAuthentication(url, type) {
  electron.ipcRenderer.send('decryptRequest', {
...
function authDecision(isAllowed, isContainerReq) {
  ...
  update();
  if (isContainerReq) {
    electron.ipcRenderer.send('registerContainerDecision', safeAuthData, isAllowed);
    return;
  }
  electron.ipcRenderer.send('registerAuthDecision', safeAuthData, isAllowed);
}
...
function render(id, page) {
  ...
  var safeNetworkStatusBtn = isSafeAppAuthenticating ? yo`<button class="toolbar-btn loading" onclick=${onClickOpenSafeAuthHome}>
        <span class="icon icon-hourglass"></span>
      </button>` : yo`<button class="toolbar-btn ${function () {
...
function onClickOpenSafeAuthHome(e) {
  setActive(create(SAFE_AUTH_DEFAULT_URL));
}
...
function onKeydownLocation(e) {
  ...
  if (e.keyCode == KEYCODE_ENTER) {
    ...
    if (page) {
      ...
      // load safeauth page
      if (new URL(selection.url).protocol === SAFE_AUTH_SCHEME) {
        if (handleSafeAuthScheme(selection.url)) {
          e.target.blur();
          return;
        }
      }
...
const SAFE_AUTH_SCHEME = 'safe-auth:';
const SAFE_AUTH_DEFAULT_URL = `${SAFE_AUTH_SCHEME}//home`;
...
function parseSafeAuthUrl(url, isClient) {
  var safeAuthUrl = {};
...
function handleSafeAuthScheme(url) {
